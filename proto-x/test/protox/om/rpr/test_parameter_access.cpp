...
BOOST_AUTO_TEST_CASE(test_om_rpr_acknowledge_param_access) {
    ...
    acknowledge.p_< OriginatingEntity >().f_< FederateIdentifier >().f_< SiteID        >() =  5;
    acknowledge.p_< OriginatingEntity >().f_< FederateIdentifier >().f_< ApplicationID >() = 10;
    acknowledge.p_< OriginatingEntity >().f_< EntityNumber       >()                       = 15;

    BOOST_CHECK(acknowledge.p_< OriginatingEntity >().f_< FederateIdentifier >().f_< SiteID        >() ==  5);
    BOOST_CHECK(acknowledge.p_< OriginatingEntity >().f_< FederateIdentifier >().f_< ApplicationID >() == 10);
    BOOST_CHECK(acknowledge.p_< OriginatingEntity >().f_< EntityNumber       >()                       == 15);

    using namespace protox::om::rpr::AcknowledgeFlagEnum16;

    acknowledge.p_< AcknowledgeFlag >() = AcknowledgeFlagEnum16::StartResume::value();

    BOOST_CHECK(acknowledge.p_< AcknowledgeFlag >() == AcknowledgeFlagEnum16::StartResume::value());
}

BOOST_AUTO_TEST_CASE(test_om_rpr_encoded_audio_radio_signal_param_access) {
    ...
    typedef i_class_type< som, q_name< RadioSignal, EncodedAudioRadioSignal > >::type encoded_radio_signal_type;

    encoded_radio_signal_type encoded_radio_signal;

    using namespace protox::om::rpr::RTIObjectIdStruct;
    encoded_radio_signal.p_< HostRadioIndex >().f_< ID >().push_back('a');
    encoded_radio_signal.p_< HostRadioIndex >().f_< ID >().push_back('b');
    encoded_radio_signal.p_< HostRadioIndex >().f_< ID >().push_back('c');

    BOOST_CHECK(encoded_radio_signal.p_< HostRadioIndex >().f_< ID >()[0] == 'a');
    BOOST_CHECK(encoded_radio_signal.p_< HostRadioIndex >().f_< ID >()[1] == 'b');
    BOOST_CHECK(encoded_radio_signal.p_< HostRadioIndex >().f_< ID >()[2] == 'c');
}
