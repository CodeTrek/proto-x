...
struct has_duplicate_class_names_impl< false, SET >
{
  ...
  typedef typename mpl::transform_view< SET, get_class_name< mpl::placeholders::_1 > >::type names;

  // Transform names into a count of each name.
  typedef typename mpl::transform_view<
    names,
    typename element_counter<names>::template op< mpl::placeholders::_1 >
  >::type name_counts;
...
{
  typedef typename has_duplicate_class_names_impl< mpl::empty<SET>::value, SET >::type type;
};
