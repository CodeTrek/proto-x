...
struct o_class_type
{
  // Construct the vector of attributes from the given qualified name.
  typedef typename o_class_attr_vector<
    typename SOM::o_class_table,
    QUALIFIED_NAME_VECTOR
  >::type attr_vector_type;

  // Construct the inheritance tree from the attribute vector.
  ...
  private:
    ...
    static void make_set( const std::string &names, std::set< std::string > &name_set )
    {
  ...
  public:
    ...
    static void publish( RTI::RTIambassador &rti_amb )
    {
      ...
      attrs_type::template collect_handles< SOM >( type::get_name(), empty_name_set, *ahs );
    ...
    static void publish( RTI::RTIambassador &rti_amb, const std::string &attrs )
    {
      ...
      attrs_type::template collect_handles< SOM >( type::get_name(), name_set, *ahs );
