...
/**
 * Compile time computation of an octet boundary value. The octet boundary value for a data type
 * is the smallest value <tt>2^n</tt>, where \c n is a non-negative integer, for which
 * <tt>(8 * 2^n) >= the size of the data type in bits.</tt>
 * \b ALWAYS invoke this meta-function with just the size in bits argument and let \c N default
 * to \c 0.
 *
