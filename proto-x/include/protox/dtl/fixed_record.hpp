...
/// \cond
/*
 * The structure of a fixed record is built using a series of derivations under the control of a reverse fold
 * meta-function. The reverse fold builds a derivation chain by applying the <tt>boost::mpl::inherit</tt> binary
 * operation to the elements of the given field vector in a pair-wise fashion.
 *
 ...
 * Note that the data type derived from this application of the reverse fold meta-function is a is essentially
 * equivalent to this POD \c struct:
...
>::type {
    ...
    public:
        ...
        /**
         * Performs a field-by-field test of this record with the given record for equality. Two records are equal if
        ...
        inline bool operator == (const fixed_record &rhs) const {
            // Construct a function that performs field-wise comparisons of the fields defined by FIELD_VECTOR.
        ...
        /**
         * Performs a field-by-field test of this record with the given record for inequality. Two records are not equal
         * if at least one field is not equal.
         *
         * \param rhs The right hand side of the inequality operator.
         */
        inline bool operator != (const fixed_record &rhs) const { return !(*this == rhs); }
         ...
         */
        template< typename T >
        inline typename T::value_type &f_() { return (static_cast< field_base< T > & >(*this).value); }
};
